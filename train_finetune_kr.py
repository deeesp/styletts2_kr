...
from customdataset import build_dataloader
...
@click.command()
@click.option('-c', '--config_path', default='Configs/config_ft.yml', type=str)
def main(config_path):
    ...
    wandb.tensorboard.patch(root_logdir=log_dir + "/tensorboard")
    wandb.init(project="styletts2-ft-kr", sync_tensorboard=True)
    ...
    train_data = data_params['train_data']
    val_data = data_params['val_data']
    ...
    train_dataloader = build_dataloader(train_data,
                                        root_path,
                                        OOD_data=OOD_data,
    ...
    val_dataloader = build_dataloader(val_data,
                                      root_path,
                                      OOD_data=OOD_data,
                                      min_length=min_length,
                                      batch_size=batch_size,
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            if (i+1)%log_interval == 0:
                logger.info ('Epoch [%d/%d], Step [%d/%d], Loss: %.5f, Disc Loss: %.5f, Dur Loss: %.5f, CE Loss: %.5f, Norm Loss: %.5f, F0 Loss: %.5f, LM Loss: %.5f, Gen Loss: %.5f, Sty Loss: %.5f, Diff Loss: %.5f, DiscLM Loss: %.5f, GenLM Loss: %.5f, SLoss: %.5f, S2S Loss: %.5f, Mono Loss: %.5f'
                    %(epoch+1, epochs, i+1, len(train_dataloader)//batch_size, running_loss / log_interval, d_loss, loss_dur, loss_ce, loss_norm_rec, loss_F0_rec, loss_lm, loss_gen_all, loss_sty, loss_diff, d_loss_slm, loss_gen_lm, s_loss, loss_s2s, loss_mono))
